...
export class EstimationEngineImpl {
    ...
    async generateProjectEstimate(requirements, options) {
        ...
        const enhancedConfidence = this.calculateConfidenceWithRequirements(complexity, requirements, historicalData || []);
        // Calculate costs
    ...
    calculateConfidenceWithRequirements(complexity, requirements, historicalData) {
        let confidence = 0.7; // Base confidence
        // Adjust based on historical data availability
        if (historicalData.length > 10) {
            confidence += 0.2;
    ...
    calculateRequirementClarity(requirements) {
        ...
        for (const req of requirements) {
            ...
            const vagueTermCount = vagueTerms.filter((term) => description.includes(term)).length;
            ...
            const specificTermCount = specificTerms.filter((term) => description.includes(term)).length;
            ...
            if (req.acceptanceCriteria && req.acceptanceCriteria.length > 0) {
                ...
                const avgCriteriaLength = req.acceptanceCriteria.reduce((sum, criteria) => sum + criteria.length, 0) / req.acceptanceCriteria.length;
    ...
    calculateCostBreakdown(timeEstimate) {
        const breakdown = timeEstimate.breakdown.map((item) => ({
            category: item.category,
            hours: item.hours,
            cost: Math.round(item.hours * this.rateConfiguration.hourlyRate * 100) / 100,
            description: item.description,
        }));
        const subtotal = breakdown.reduce((sum, item) => sum + item.cost, 0);
        const overhead = Math.round(subtotal * this.rateConfiguration.overhead * 100) / 100;
        const profit = Math.round((subtotal + overhead) * this.rateConfiguration.profitMargin * 100) / 100;
    ...
    async generateTimeEstimateWithBuffer(complexity, bufferPercentage = 0.2, historicalData, options) {
        const baseEstimate = await this.generateTimeEstimate(complexity, historicalData, options);
        const buffer = Math.round(baseEstimate.totalHours * bufferPercentage * 100) / 100;
        const totalWithBuffer = Math.round((baseEstimate.totalHours + buffer) * 100) / 100;
        return {
            ...baseEstimate,
            buffer,
            totalWithBuffer,
    ...
    async generateResourceBasedEstimate(requirements, teamConfiguration, options) {
        ...
        const totalTeamMembers = teamConfiguration.seniorDevelopers +
        ...
        const resourceBreakdown = {
            senior: {
                ...
                cost: Math.round(seniorHours * teamConfiguration.seniorRate * 100) / 100,
            ...
            junior: {
                ...
                cost: Math.round(juniorHours * teamConfiguration.juniorRate * 100) / 100,
                developers: teamConfiguration.juniorDevelopers,
            },
        };
        const totalResourceCost = resourceBreakdown.senior.cost +
        ...
        return {
            ...
            totalCost: Math.round(totalResourceCost *
