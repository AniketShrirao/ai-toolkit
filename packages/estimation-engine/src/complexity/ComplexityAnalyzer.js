export class ComplexityAnalyzer {
    ...
    historicalData;
    ollamaService;
    constructor(ollamaService, initialFactors) {
    ...
    async calculateComplexity(requirements, options) {
        ...
        for (const requirement of requirements) {
            const reqComplexity = await this.analyzeRequirementComplexity(requirement);
        ...
        const overall = Math.min(10, Math.max(1, baseOverall * historicalAdjustment));
        return {
            overall,
    ...
    buildComplexityAnalysisPrompt(requirement, context) {
...
Score:`;
    ...
    parseComplexityScore(response) {
        ...
        if (text.includes("complex") || text.includes("difficult"))
            return 7;
        if (text.includes("very complex") || text.includes("extremely"))
            return 9;
        if (text.includes("moderate") || text.includes("medium"))
            return 5;
        return 5; // Default moderate complexity
    }
    calculateHeuristicComplexity(requirement) {
        ...
        if (complexityKeywords.veryComplex.some((keyword) => description.includes(keyword))) {
            score += 4;
        }
        else if (complexityKeywords.complex.some((keyword) => description.includes(keyword))) {
            score += 3;
        }
        else if (complexityKeywords.moderate.some((keyword) => description.includes(keyword))) {
            score += 1;
        }
        return Math.min(10, Math.max(1, score));
    }
    async identifyComplexityFactors(requirement) {
        const factors = [];
    ...
    isTechnicalRequirement(requirement) {
        ...
        return technicalKeywords.some((keyword) => requirement.description.toLowerCase().includes(keyword));
    }
    ...
    requiresExtensiveTesting(requirement) {
        ...
        return testingKeywords.some((keyword) => requirement.description.toLowerCase().includes(keyword));
    }
    getHistoricalAdjustment(requirements) {
        ...
        const accuracyRatios = similarProjects.map((project) => project.actualHours / project.estimatedHours);
        const avgRatio = accuracyRatios.reduce((sum, ratio) => sum + ratio, 0) /
    ...
    findSimilarProjects(requirements) {
        ...
        return this.historicalData.filter((project) => {
            ...
            const commonKeywords = projectKeywords.filter((keyword) => currentKeywords.includes(keyword));
