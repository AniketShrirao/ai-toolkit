...
export class WorkflowDefinitionLoader {
    ...
    constructor(configPath = "./workflows.json") {
    ...
    async loadWorkflows() {
        try {
            ...
            if (!this.isCompatibleVersion(config.version)) {
                throw new Error(`Incompatible workflow config version: ${config.version}`);
            }
            // Process and validate workflows
            const workflows = await this.processWorkflows(config.workflows, config.templates);
    ...
    async processWorkflows(workflows, templates) {
        const processed = [];
        for (const workflow of workflows) {
            // Apply templates if specified
            let processedWorkflow = { ...workflow };
            if (workflow.steps) {
                processedWorkflow.steps = await this.processSteps(workflow.steps, templates);
            }
            // Set default values
    ...
    async processSteps(steps, templates) {
        const processed = [];
    ...
    validateWorkflowDefinition(workflow, index) {
        const errors = [];
    ...
    createDocumentProcessingWorkflow(name, description) {
        return {
    ...
    createEstimationWorkflow(name, description) {
        return {
...
//# sourceMappingURL=WorkflowDefinitionLoader.js.map