...
import {
  ...
  ValidationError,
  ValidationWarning
} from '../interfaces/PromptManager.js';

export class PromptManagerImpl implements PromptManager {
  private templates: Map<string, PromptTemplate> = new Map();
  private readonly defaultContext: PromptContext = {
    maxTokens: 4096,
    currentTokens: 0,
...
Format as structured analysis with clear sections and actionable recommendations.`,
  ...
  private processVariables(template: PromptTemplate, variables: Record<string, any>): Record<string, any> {
    ...
    if (processed.length && !processed.lengthDescription) {
      ...
      processed.lengthDescription = lengthDescriptions[processed.length as keyof typeof lengthDescriptions] || '1-2 paragraphs';
    }
  ...
  truncateContent(
    content: string,
    maxTokens: number,
    strategy: 'start' | 'middle' | 'end' | 'smart' = 'smart'
  ): string {
  ...
  private scoreLineImportance(line: string, keywords: string[]): number {
    let score = 0;
  ...
  async optimizePrompt(content: string, targetLength?: number): Promise<PromptOptimization> {
